...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--num_runs', type=int, default=1, help='number of runs') # change to 5
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        if args.model_name == 'JODIE':
            if args.dataset_name in ['ICEWS1819']:
                args.dropout = 0.1
            elif args.dataset_name in ['mooc', 'USLegis']:
                args.dropout = 0.2
        ...
        if args.dataset_name in ['wikipedia', 'reddit', 'enron']:
            ...
            args.dropout = 0.1
        if args.dataset_name in ['CanParl', 'UNtrade', 'UNvote', 'ICEWS1819']:
            args.sample_neighbor_strategy = 'uniform'
        else:
            args.sample_neighbor_strategy = 'recent'
...
def get_node_classification_args():
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='Cite_HepTH', choices=['Amazon_movies', 'Enron', 'GDELT', 'Googlemap_CT', 'ICEWS1819', 'Stack_elec', 'Stack_english', 'Stack_ubuntu', 'Yelp', 'Cite_HepTH'])
...
def get_node_classification_args():
    ...
    # arguments
    parser = argparse.ArgumentParser('Interface for the node classification task')
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='Cite_HepTH', choices=['wikipedia', 'reddit'])
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer'])
    parser.add_argument('--gpu', type=int, default=0, help='number of gpu to use')
